...
- 21/12 - Je remarque trop de redondances sur le code de JeuGraphique donc j'ajoute des méthodes directement (freze() et defreeze()) + pour un fonctionnement plus optimal du piege j'ai déplacé les attributs de pv dans la classe Ball. Motivation : imaginons qu'on s'est déjà pris un piège mais qu'après on tombe dans un trou, on ne peut donc pas réinitialiser les pv de la case 
